...
Let time index be \(t\), meteorological feature vector be \(\mathbf{m}_t \in \mathbb{R}^F\), satellite frame be \(\mathbf{I}_t \in \mathbb{R}^{1\times H\times W}\), and precipitation value be \(p_t\).
...
with \(\max(\sigma_j,10^{-6})\) for numerical safety.
...
with learning rate \(\nu\), weak learner \(h_m\), and step size \(\gamma_m\).
...
\(*\) is convolution, \(\odot\) is elementwise multiplication.

### 5.2 Meteorological LSTM Branch and Fusion Head

Let \(\mathbf{h}^{\text{meteo}}\) be final hidden state of the meteorological LSTM and \(\mathbf{h}^{\text{img}}\) be pooled image feature.
...
In code, \(w_+ \approx \frac{N_{\text{neg}}}{N_{\text{pos}}}\) (lower-bounded by 1).
...
- ReduceLROnPlateau: if validation loss stagnates, \(\eta \leftarrow 0.5\eta\).
...
For metric \(m_k\) over runs \(r=1,\ldots,R\):
...
Training is considered stable when \(\Delta_{\max}\le \delta\) (project default \(\delta=0.03\)).
...
| Stability criterion \(\Delta_{\max}\) | `src/train/train.py` |
